...
////////////////////////////// FMGGridSceneCell //////////////////////////////
void FMGGridSceneCell::Add(const FDungeonMarkerInstance& InMarkerInfo, EMarkerGenGridPatternRuleType InType, EMGMarkerListDuplicateCheckMethod DuplicateCheckMethod) {
	// Make sure we don't have duplicates
	if (TArray<FDungeonMarkerInstance>* MarkerList = GetMarkerList(InType)) {
		bool bFoundDuplicate = false;
		if (DuplicateCheckMethod != EMGMarkerListDuplicateCheckMethod::None) {
			for (const FDungeonMarkerInstance& MarkerInfo : *MarkerList) {
...
void FMGGridSceneCell::Remove(const FString& InMarkerName, EMarkerGenGridPatternRuleType InType) {
	TArray<FDungeonMarkerInstance>* MarkerList = GetMarkerList(InType);
	if (MarkerList) {
		*MarkerList = MarkerList->FilterByPredicate([InMarkerName](const FDungeonMarkerInstance& MarkerInfo) -> bool {
...
bool FMGGridSceneCell::Contains(const FString& InMarkerName, EMarkerGenGridPatternRuleType InType) const {
	const TArray<FDungeonMarkerInstance>* MarkerList = GetMarkerList(InType);
	if (MarkerList) {
		for (const FDungeonMarkerInstance& MarkerInfo : *MarkerList) {
...
TArray<FDungeonMarkerInstance>* FMGGridSceneCell::GetMarkerList(EMarkerGenGridPatternRuleType InType) {
	TArray<FDungeonMarkerInstance>* MarkerList = nullptr;
...
const TArray<FDungeonMarkerInstance>* FMGGridSceneCell::GetMarkerList(EMarkerGenGridPatternRuleType InType) const {
	const TArray<FDungeonMarkerInstance>* MarkerList = nullptr;
	if (InType == EMarkerGenGridPatternRuleType::Ground) {
		MarkerList = &TileMarkers;
	}
	else if (InType == EMarkerGenGridPatternRuleType::Corner) {
...
FMGGridSceneMarkerList::FMGGridSceneMarkerList(const FVector& InCellSize, const TArray<FDungeonMarkerInstance>& InMarkers, int32 InBoundsExpansion)
...
void FMGGridSceneMarkerList::GenerateMarkerList(TArray<FDungeonMarkerInstance>& OutMarkers) const {
	OutMarkers.Reset();
